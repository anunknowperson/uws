...
CGINCLUDE
	...
	v2f vert(appdata_full v)
	{
		...
		o.normalInterpolator.xyz = float3(0.0, 1.0, 0.0);
	...
	inline half3 ComputeNormalUsingFlowMap(float2 flowMap, half3 normal, float2 uv, float time, float3 vwn, float ppd)
	{
		...
		half3 tex1 = PerPixelNormal(_BumpMap, float4(uv - 0.25 * flowMap * time1, 0.0, 0.0), vwn,  ppd);
		half3 tex2 = PerPixelNormal(_BumpMap, float4(uv - 0.25 * flowMap * time2, 0.0, 0.0), vwn,  ppd);
		half3 flowNormal = lerp(tex1, tex2, flowLerp);
		flowNormal.xz *= flowLerpFix;
		return lerp(normal, flowNormal, saturate(blendMask));
	}
	
	inline half3 GetFlowmapNormal(float3 worldPos, float2 uv, half3 normal, float3 vwn, float ppd)
	{
		...
		return ComputeNormalUsingFlowMap(flowmap, normal, uv, _Time.y * _FlowSpeedScale, vwn, ppd);
	}
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		#if FLOW_FLOWMAP 
			worldNormal = GetFlowmapNormal(i.worldPos, i.bumpCoords, worldNormal, VERTEX_WORLD_NORMAL, PER_PIXEL_DISPLACE);
		#endif
