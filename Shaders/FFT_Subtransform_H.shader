Shader "Hidden/FFT_Subtransform_H"
{
    SubShader
    {
        Pass
        {
            ...
            {
			    float index = i.uv.x * transformSize - 0.5;

			    float evenIndex = floor(index / subtransformSize) * (subtransformSize * 0.5) + fmod(index, subtransformSize * 0.5);
                
			    float4 even = tex2D(input, float2(evenIndex + 0.5, i.uv.y * transformSize) / transformSize).rgba;
			    float4 odd = tex2D(input, float2(evenIndex + transformSize * 0.5 + 0.5, i.uv.y * transformSize) / transformSize).rgba;
