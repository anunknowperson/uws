
Shader "Hidden/UltraMobileWater" {
Properties {
	_horizonColor ("Horizon color", COLOR)  = ( .172 , .463 , .435 , 0)
	_WaveScale ("Wave scale", Range (0.02,0.15)) = .07
	[NoScaleOffset] _ColorControl ("Reflective color (RGB) fresnel (A) ", 2D) = "" { }
	[NoScaleOffset] _BumpMap ("Waves Normalmap ", 2D) = "" { }
	WaveSpeed ("Wave speed (map1 x,y; map2 x,y)", Vector) = (19,9,-16,-7)
	}

CGINCLUDE

#include "UnityCG.cginc"

uniform float4 _horizonColor;

uniform float4 WaveSpeed;
uniform float _WaveScale;
uniform float4 _WaveOffset;

sampler2D _Flowmap;
float2 _FlowmapPosition;
float2 _FlowmapScale;
float _FlowSpeedScale;

struct appdata {
	float4 vertex : POSITION;
	float3 normal : NORMAL;
};

struct v2f {
	float4 pos : SV_POSITION;
	float2 bumpuv[2] : TEXCOORD0;
	
	float3 viewDir : TEXCOORD2;
	float3 worldPos :TEXCOORD3;
	UNITY_FOG_COORDS(3)
};

v2f vert(appdata v)
{
	v2f o;
	float4 s;

	o.pos = UnityObjectToClipPos(v.vertex);

	// scroll bump waves
	float4 temp;
	float4 wpos = mul (unity_ObjectToWorld, v.vertex);
	o.worldPos = wpos.xyz;

	temp.xyzw = wpos.xzxz * _WaveScale + WaveSpeed * _Time.y / 40.0;
	o.bumpuv[0] = temp.xy * float2(.4, .45);
	o.bumpuv[1] = temp.wz;

	// object space view direction
	o.viewDir.xzy = normalize( WorldSpaceViewDir(v.vertex) );

	UNITY_TRANSFER_FOG(o,o.pos);
	return o;
}

ENDCG


Subshader {
	Tags { "RenderType"="Opaque" "Queue"="Transparent"}
	Cull Off
	Pass {

CGPROGRAM
#pragma vertex vert
#pragma fragment frag
#pragma multi_compile_fog
#pragma multi_compile _ FLOW_FLOWMAP

sampler2D _BumpMap;
sampler2D _ColorControl;

inline half3 ComputeNormalUsingFlowMap(float2 flowMap, half3 normal, float2 uv, float time)
{
	half blendMask = abs(flowMap.x) + abs(flowMap.y);
	//if (blendMask < 0.01) return normal;

	half time1 = frac(time + 0.5);
	half time2 = frac(time);
	half flowLerp = abs((0.5 - time1) / 0.5);
	half flowLerpFix = lerp(1, 0.65, abs(flowLerp * 2 - 1)); //fix interpolation bug, TODO: I need find what cause this. 
	
	half3 tex1 = UnpackNormal(tex2D(_BumpMap, float4(uv - 0.25 * flowMap * time1, 0.0, 0.0))).rgb;
	half3 tex2 = UnpackNormal(tex2D(_BumpMap, float4(uv - 0.25 * flowMap * time2, 0.0, 0.0))).rgb;
	half3 flowNormal = lerp(tex1, tex2, flowLerp);
	flowNormal.xz *= flowLerpFix;
	return lerp(normal, flowNormal, saturate(blendMask));
}

inline half3 GetFlowmapNormal(float3 worldPos, float2 uv, half3 normal)
{
	float2 flowMapUV =  (worldPos.xz - _FlowmapPosition) / _FlowmapScale - float2(0.5, 0.5);
	float2 flowmap = tex2Dlod(_Flowmap, float4(flowMapUV, 0.0, 0.0)) * 2 - 1;
	return ComputeNormalUsingFlowMap(flowmap, normal, uv, _Time.y * _FlowSpeedScale);
}

half4 frag( v2f i ) : COLOR
{
	half3 bump1 = UnpackNormal(tex2D( _BumpMap, i.bumpuv[0] )).rgb;
	half3 bump2 = UnpackNormal(tex2D( _BumpMap, i.bumpuv[1] )).rgb;

	#if FLOW_FLOWMAP 
		bump1 = GetFlowmapNormal(i.worldPos, i.bumpuv[0], bump1);
		bump2 = GetFlowmapNormal(i.worldPos, i.bumpuv[1], bump1);
	#endif

	half3 bump = (bump1 + bump2) * 0.5;
	
	half fresnel = dot( i.viewDir, bump );
	half4 water = tex2D( _ColorControl, float2(fresnel,fresnel) );
	
	half4 col;
	col.rgb = lerp( water.rgb, _horizonColor.rgb, water.a );
	col.a = _horizonColor.a;

	UNITY_APPLY_FOG(i.fogCoord, col);
	return col;
}
ENDCG
	}
}

}
